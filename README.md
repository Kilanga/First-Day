...
1. The subject route creates the mentor, subject, hire, and one `ConceptState` per generated concept.
...
1. Create a subject and optional notes from `/onboarding`.
...
8. Start another session on the same subject or a new subject from the dashboard.
