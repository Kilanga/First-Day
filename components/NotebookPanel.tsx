...
export default function NotebookPanel({ name, concepts, onClose }: { name: string; concepts: NotebookConcept[]; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 bg-slate-950/25 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${name}'s field notes`}>
    <aside className="ml-auto flex h-full w-full max-w-md flex-col overflow-y-auto bg-white p-6 shadow-2xl sm:p-8">
      <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-700">{name}&apos;s field notes</p><h2 className="font-display mt-2 text-3xl font-semibold text-[#111827]">What is taking shape</h2><p className="mt-2 text-sm leading-6 text-slate-500">Each page follows one idea in {name}&apos;s own learning journey.</p></div><button onClick={onClose} className="rounded-full px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-amber-50">Close</button></div>
