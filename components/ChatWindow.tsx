...
export default function ChatWindow({ subjectId, hire, initialQuestion, initialSessionId, initialMessages, onHireUpdate }: Props) {
  ...
  useEffect(() => {
    if (!subjectId) return;
    try { setTeachingChecks(JSON.parse(localStorage.getItem(`first-day-teaching-checks:${subjectId}`) ?? "{}") as Record<string, boolean>); } catch { setTeachingChecks({}); }
  }, [subjectId]);
  ...
  async function deliver(message: string, appendMessage: boolean) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.status === 429 ? "The study room is closed for today — come back tomorrow." : data.error ?? "Unable to send the message.");
  ...
  return <section aria-label="Mentor conversation" className="flex min-h-[520px] flex-col rounded-2xl border border-indigo-100 bg-white shadow-sm">
    ...
    <form onSubmit={send} className="border-t border-slate-100 p-4"><div className="flex gap-3"><label className="sr-only" htmlFor="mentor-explanation">Explain your answer</label><textarea id="mentor-explanation" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} maxLength={6000} rows={2} disabled={thinking} aria-describedby="mentor-explanation-help mentor-explanation-count" placeholder="Explain it to your study partner…" className="min-h-[52px] flex-1 resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:bg-slate-50" /><button type="submit" disabled={!draft.trim() || thinking} className="rounded-xl bg-indigo-600 px-4 text-sm font-semibold text-white transition hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40 sm:px-5">Send</button></div><div className="mt-2 flex flex-wrap justify-between gap-x-3 gap-y-1 text-xs text-slate-400"><p id="mentor-explanation-help">Press Ctrl/⌘ + Enter to send</p><p id="mentor-explanation-count">{draft.length.toLocaleString()} / 6,000</p></div><div className="mt-3"><button type="button" aria-expanded={stuckHelpOpen} onClick={() => setStuckHelpOpen((open) => !open)} className="text-xs font-semibold text-indigo-700 underline underline-offset-2 hover:text-indigo-900">{stuckHelpOpen ? "Hide study checklist" : "I’m stuck — help me structure this"}</button>{stuckHelpOpen ? <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-3 text-xs leading-5 text-slate-700"><p className="font-semibold text-indigo-900">Private study checklist</p><div className="mt-2 grid gap-1 sm:grid-cols-2">{[["definition", "Plain-language definition"], ["why", "Why it matters"], ["example", "Concrete example"], ["edge", "Useful edge case"]].map(([key, label]) => <label key={key} className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={Boolean(teachingChecks[key])} onChange={() => toggleTeachingCheck(key)} className="accent-indigo-600" />{label}</label>)}</div><p className="mt-3 text-slate-500">Aim for the next useful point, not a perfect lecture. This checklist stays only in this browser.</p></div> : null}</div>{error ? <div role="alert" className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-rose-50 px-3 py-2 text-xs text-rose-700"><span>{error}</span>{lastFailedMessage ? <button type="button" onClick={() => void deliver(lastFailedMessage, false)} disabled={thinking} className="font-semibold underline underline-offset-2 hover:text-rose-900 disabled:opacity-50">Try again</button> : null}</div> : null}</form>
  </section>;
}
