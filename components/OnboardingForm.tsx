...
export default function OnboardingForm() {
  ...
  useEffect(() => {
    void ensureMentorSession().then(() => setSessionReady(true)).catch(() => setError("Unable to open your private learning desk. Please refresh and try again."));
  }, []);
  ...
  async function createSubject(event: FormEvent) {
    ...
    setLoading(true);
    setError(undefined);
    setStage(files.length ? "Reading your learning material..." : "Preparing your learning path...");
    const questionTimer = window.setTimeout(() => setStage("Your learning partner is getting ready..."), 900);
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "Unable to start this learning path.");
      setCreated(data);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Unable to start this learning path.");
    } finally {
