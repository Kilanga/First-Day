...
export default function OnboardingForm() {
  ...
  if (created) return <section className="rounded-2xl border border-indigo-100 bg-white p-8 text-center shadow-sm">
    ...
    {created.status === "preparing" ? <div className="mx-auto mt-6 flex max-w-sm items-center justify-center gap-3 rounded-xl bg-indigo-50 px-4 py-3 text-sm font-medium text-indigo-700"><span className="h-4 w-4 animate-spin rounded-full border-2 border-indigo-200 border-t-indigo-600" /><WaitingMessage /></div>
      : created.status === "failed" ? <div className="mt-6"><p role="alert" className="text-sm text-rose-600">{created.error ?? "We could not prepare this onboarding plan."}</p><button onClick={() => void retryPreparation()} className="mt-4 rounded-xl border border-indigo-200 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50">Try again</button></div>
        : <button onClick={enterOffice} className="mt-7 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white">Enter the office</button>}
  </section>;
  ...
  return <form onSubmit={createSubject} className="rounded-2xl border border-indigo-100 bg-white p-7 shadow-sm">
    ...
    <div className="mt-5">
      <p className="text-sm font-semibold text-slate-800">Add onboarding material <span className="font-normal text-slate-400">(optional)</span></p>
      ...
      <p className="mt-2 text-xs leading-5 text-slate-400">Text is used only to prepare this onboarding plan. It is removed once preparation succeeds; failed drafts stay available only so you can retry or delete them. 3 MB per file, 4 MB total.</p>
    ...
    <button disabled={!title.trim() || !sessionReady || loading || (files.length > 0 && !focus.trim())} className="mt-6 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{loading ? "Preparing your onboarding plan..." : sessionReady ? "Meet your new hire" : "Opening your onboarding desk..."}</button>
