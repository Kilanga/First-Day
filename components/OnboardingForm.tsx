  ...
  function addFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (selected.length) {
      setFiles((current) => {
        ...
        const newFiles = selected.filter((file) => !known.has(`${file.name}:${file.size}:${file.lastModified}`));
        return [...current, ...newFiles].slice(0, 4);
      });
    }
    // Re-mounting allows the same file to be picked again after it is removed.
