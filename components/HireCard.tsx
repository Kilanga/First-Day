...
export default function HireCard({ hire, concepts = [], xpFloat, compact = false, breakthrough = false, onOpenFieldNotes }: Props) {
  ...
  return <aside className="relative rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm">
    ...
    <section className="mt-7"><div className="mb-2 flex justify-between text-sm"><span className="font-medium text-slate-700">Learning progress</span><span className="text-slate-500">{hire.xp} points{hire.tier === "confirmed" ? "" : ` / ${target}`}</span></div><div className="h-2.5 overflow-hidden rounded-full bg-indigo-100"><div className="h-full rounded-full bg-indigo-600 transition-all duration-700 ease-out" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs text-slate-500">{nextTier ? `${remaining} points until ${nextTier}` : "Ready to help others learn"}</p><p className="mt-1 text-xs text-slate-400">Explorer: 0–50 · Builder: 51–150 · Guide: 151+</p></section>
    <section className="mt-8"><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold text-slate-800">Concept map</h3><span className="text-xs text-slate-400">Current picture</span></div>{concepts.length ? <div className="space-y-2">{concepts.map((concept) => <div key={concept.id} className="flex items-center gap-3"><span className="min-w-0 flex-1 truncate text-xs text-slate-600">{concept.name}</span><span title={concept.status === "mastered" ? `${concept.name}: ${concept.notebookEntry ?? "Understood"}` : `${concept.name}: ${concept.status === "not_covered" ? "Not explored yet" : "In progress"}`} className={`h-4 w-7 rounded-md border ${concept.status === "mastered" ? "border-emerald-400 bg-emerald-400" : concept.status === "not_covered" ? "border-slate-200 bg-slate-100" : "border-amber-300 bg-amber-300"}`} /></div>)}</div> : <p className="text-xs text-slate-400">Concepts will take shape as you learn together.</p>}<p className="mt-4 text-xs text-slate-400">Teaching signals: {Object.entries(hire.stats).map(([key, value]) => `${STAT_LABELS[key as keyof typeof STAT_LABELS]} ${value}`).join(" · ")}</p></section>
    {onOpenFieldNotes ? <button onClick={onOpenFieldNotes} className="mt-5 w-full rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-900 transition hover:bg-amber-100">Open {hire.name}&apos;s field notes</button> : null}
  </aside>;
}
