...
export function tierLabel(tier: string) { return TIER_LABELS[tier] ?? "Week 1"; }
...
export default function HireCard({ hire, concepts = [], xpFloat, compact = false, breakthrough = false, onOpenFieldNotes }: Props) {
  ...
  return <aside className="surface-card relative p-6">
    ...
    <section className="mt-7"><div className="mb-2 flex justify-between text-sm"><span className="font-medium text-slate-700">Onboarding progress</span><span className="text-slate-500">{hire.xp} XP{hire.tier === "confirmed" ? "" : ` / ${target}`}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#F3F4F6]"><div className="h-full rounded-full bg-[#4F46E5] transition-all duration-700 ease-out" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs text-slate-500">{nextTier ? `${remaining} XP until ${nextTier}` : "Ready for the probation review"}</p><p className="mt-1 text-xs text-slate-400">Week 1: 0–50 · First Month: 51–150 · Confirmed: 151+</p></section>
