...
const STUDY_PATH_MESSAGES = [
  "Sketching the key ideas…",
  "Looking for the questions worth asking…",
  "Connecting the building blocks…",
  "Finding the misconceptions that make learning stick…",
  "Turning the topic into a path your partner can explore…",
  ...
  "Giving the study path a clear shape…",
  "Saving the best questions for your next session…",
  "Making sure your partner starts in the right place…",
  "Almost ready to learn together…",
];

export default function WaitingMessage() {
  const [index, setIndex] = useState(0);
  useEffect(() => { const timer = window.setInterval(() => setIndex((current) => (current + 1) % STUDY_PATH_MESSAGES.length), 10_000); return () => window.clearInterval(timer); }, []);
  return <span aria-live="polite">{STUDY_PATH_MESSAGES[index]}</span>;
}
