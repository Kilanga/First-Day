...
type WorkspaceSubject = { id: string; title: string; firstQuestion?: string; hire: HireView; concepts: SkillConcept[]; progress?: { explored: number; mastered: number; toRevisit: number; total: number }; activeSession: ActiveSession | null };
type Props = { subjectId?: string; title: string; name: string; initialQuestion?: string };

function subjectUrl(subject: WorkspaceSubject) { return `/office?${new URLSearchParams({ subjectId: subject.id, title: subject.title, hireName: subject.hire.name, ...(subject.firstQuestion ? { firstQuestion: subject.firstQuestion } : {}) })}`; }
function agendaIds(value: unknown) { return value && typeof value === "object" && Array.isArray((value as { conceptIds?: unknown }).conceptIds) ? (value as { conceptIds: unknown[] }).conceptIds.filter((id): id is string => typeof id === "string") : []; }
...
export default function OfficeWorkspace({ subjectId, title, name, initialQuestion }: Props) {
  ...
  const [xpFloat, setXpFloat] = useState(0); const [tierUp, setTierUp] = useState(false); const [breakthrough, setBreakthrough] = useState(false);
  const [agendaComplete, setAgendaComplete] = useState(false); const [ending, setEnding] = useState(false); const [sessionId, setSessionId] = useState<string>();
  const [report, setReport] = useState<Report>(); const [reportOpen, setReportOpen] = useState(false); const [loadingReport, setLoadingReport] = useState(false); const [snapshotCount, setSnapshotCount] = useState<number>(); const [confirmEnd, setConfirmEnd] = useState(false); const [notebookOpen, setNotebookOpen] = useState(false);
  ...
  function updateHire(nextHire: HireView, xpDelta: number, crossedTier: boolean, nextSessionId: string, didBreakthrough: boolean, finishedAgenda: boolean) {
    ...
    if (didBreakthrough) { setBreakthrough(true); window.setTimeout(() => setBreakthrough(false), 2200); }
    if (finishedAgenda) { setAgendaComplete(true); window.setTimeout(() => setAgendaComplete(false), 4600); }
    void refreshSubjects().catch(() => undefined);
  }

  async function endSession() { if (!sessionId || ending) return; setEnding(true); try { const response = await fetch("/api/session/end", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Unable to end the session."); router.push(`/report/${data.sessionId}`); } catch (error) { window.alert(error instanceof Error ? error.message : "Unable to end the session."); setEnding(false); } }
  async function viewReport() { if (!sessionId || loadingReport) return; setLoadingReport(true); try { const response = await fetch("/api/session/end", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, preview: true }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Unable to prepare the report."); setReport(data.report); setSnapshotCount(data.snapshotMessageCount); setReportOpen(true); } catch (error) { window.alert(error instanceof Error ? error.message : "Unable to prepare the report."); } finally { setLoadingReport(false); } }

  return <main className="min-h-screen bg-[#faf9f7] text-slate-800">
    <header className="border-b border-indigo-100 bg-white"><div className="mx-auto max-w-6xl px-5 py-4 sm:px-8"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-600">First Day</p><h1 className="mt-1 text-lg font-semibold text-slate-900">{currentTitle}</h1></div><div className="flex items-center gap-1 sm:gap-2"><button onClick={() => setNotebookOpen(true)} className="rounded-xl px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50">{hire.name}&apos;s field notes</button><button onClick={() => router.push("/")} className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">Leave</button><button onClick={viewReport} disabled={loadingReport || !sessionId} className="rounded-xl px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 disabled:opacity-50">{loadingReport ? "Preparing…" : "View report"}</button><button onClick={() => setConfirmEnd(true)} disabled={ending || !sessionId} className="rounded-xl border border-indigo-200 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 disabled:opacity-50">{ending ? "Ending…" : "End session"}</button></div></div><nav className="mt-4 flex gap-2 overflow-x-auto pb-1">{subjects.map((subject) => <button key={subject.id} onClick={() => router.push(subjectUrl(subject))} className={`shrink-0 rounded-xl px-3 py-2 text-sm font-semibold transition ${subject.id === subjectId ? "bg-indigo-600 text-white" : "bg-indigo-50 text-indigo-700 hover:bg-indigo-100"}`}>{subject.title} — {subject.hire.name}{subject.activeSession ? <span className="ml-2 inline-block h-1.5 w-1.5 rounded-full bg-current align-middle" /> : null}</button>)}<button onClick={() => router.push("/onboarding")} className="shrink-0 rounded-xl border border-dashed border-indigo-300 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50">+ New subject</button></nav></div></header>
    <div className="mx-auto max-w-6xl px-5 py-6 sm:px-8 lg:py-10"><div className="mb-5 md:hidden"><HireCard hire={hire} concepts={currentSubject?.concepts} compact /></div>{tierUp ? <div className="mb-5 rounded-2xl border border-indigo-200 bg-indigo-50 px-5 py-3 text-center text-sm font-semibold text-indigo-800">🎉 {hire.name} is now a {tierLabel(hire.tier)}!</div> : null}{breakthrough ? <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3 text-center text-sm font-semibold text-amber-900">✨ A real breakthrough — {hire.name} finally connected the dots.</div> : null}{agendaComplete ? <div className="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 text-center text-sm font-semibold text-emerald-900">“That was a great study session — I got everything on my list!”</div> : null}<div className="grid items-start gap-6 md:grid-cols-3"><div className="md:col-span-2"><ChatWindow subjectId={subjectId} hire={hire} initialQuestion={currentQuestion} initialSessionId={currentSubject?.activeSession?.id} initialMessages={currentSubject?.activeSession?.messages} onHireUpdate={updateHire} /></div><div className="space-y-6"><section className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-indigo-600">Today&apos;s study plan</p>{agenda.length ? <ul className="mt-4 space-y-3">{agenda.map((concept) => <li key={concept.id} className="flex items-center gap-3 text-sm text-slate-700"><span className={`grid h-5 w-5 place-items-center rounded-full text-xs ${concept.status === "mastered" ? "bg-emerald-500 text-white" : "border border-slate-300 text-transparent"}`}>✓</span>{concept.name}</li>)}</ul> : <p className="mt-3 text-sm text-slate-500">Send a first message and {hire.name}&apos;s study plan will appear here.</p>}</section><div className="hidden md:block"><HireCard hire={hire} concepts={currentSubject?.concepts} xpFloat={xpFloat} breakthrough={breakthrough} /></div></div></div></div>
