...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setId(mentor);
    fetch(`/api/state?mentorId=${encodeURIComponent(mentor)}`)
  ...
  function openPrimary(subject: Subject) {
    if (subject.activeSession) return openOffice(subject);
    if (subject.latestCompletedSession) return router.push(`/report/${subject.latestCompletedSession.id}`);
    openOffice(subject);
  }
  ...
  async function deleteSubject(subject: Subject) {
    ...
    try {
      const response = await fetch("/api/subjects", {
        ...
        body: JSON.stringify({ mentorId: id, subjectId: subject.id }),
      });
