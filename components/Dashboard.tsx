...
export default function Dashboard() {
  ...
  useEffect(() => {
    void ensureMentorSession().then(() => fetch("/api/state")).then(async (response) => response.ok ? response.json() : { subjects: [] }).then((data) => setSubjects(data.subjects ?? [])).catch(() => setSubjects([]));
  }, []);
  ...
  async function deleteLearningDesk() {
    ...
    try {
      ...
      clearLocalMentorId(); window.location.assign("/");
    } catch (error) { window.alert(error instanceof Error ? error.message : "Unable to delete your learning desk."); setDeletingDesk(false); }
  }
