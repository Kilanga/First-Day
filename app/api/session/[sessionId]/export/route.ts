...
export async function GET(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const markdown = [`# First Day session report`, ``, `**Subject:** ${session.subject.title}`, `**New hire:** ${session.subject.hire?.name ?? "New hire"}`, `**Completed:** ${session.endedAt?.toISOString().slice(0, 10) ?? "In progress"}`, ``, `## What landed well`, ...strengths.map((item) => `- ${item}`), ``, `## Ideas to sharpen`, ...gaps.flatMap((gap) => [`### ${gap.concept}`, gap.whatWasMissing, `**Next time:** ${gap.tryNextTime}`, ``]), `## Suggested next session`, next, ``].join("\n");
