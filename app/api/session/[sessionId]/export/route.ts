...
export async function GET(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const markdown = [`# First Day study reflection`, ``, `**Subject:** ${session.subject.title}`, `**Study partner:** ${session.subject.hire?.name ?? "Study partner"}`, `**Completed:** ${session.endedAt?.toISOString().slice(0, 10) ?? "In progress"}`, ``, `## What landed well`, ...strengths.map((item) => `- ${item}`), ``, `## Ideas to revisit`, ...gaps.flatMap((gap) => [`### ${gap.concept}`, gap.whatWasMissing, `**Next time:** ${gap.tryNextTime}`, ``]), `## Suggested next study session`, next, ``].join("\n");
