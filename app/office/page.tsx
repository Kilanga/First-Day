...
export default async function OfficePage({ searchParams }: { searchParams: Promise<{ subjectId?: string; title?: string; hireName?: string; firstQuestion?: string }> }) {
  ...
  return <OfficeWorkspace subjectId={query.subjectId} title={query.title ?? "Your learning session"} name={query.hireName ?? "Your learning partner"} initialQuestion={query.firstQuestion} />;
}
