...
export async function refreshSubjectGeneration(subjectId: string): Promise<GenerationState> {
  ...
  if (!subject) return { status: "failed", error: "This study path no longer exists." };
  ...
  if (!subject.generationResponseId) return { status: "failed", error: "The study path could not be started. Try again." };
  ...
  if (response.status !== "completed") {
    const error = "We could not finish preparing this study path. Try again.";
  ...
  try {
    ...
    const error = "We could not prepare this study path. Try again.";
    await prisma.subject.update({ where: { id: subject.id }, data: { generationStatus: "failed", generationError: error, generationResponseId: null } });
    return { status: "failed", error };
  }
}
