...
export async function generateMentorFeedback(name: string, messages: Array<{ role: string; verdict: Prisma.JsonValue | null }>) {
  ...
  return (await callText(
    `You are ${name}, a junior colleague giving warm, specific, professional feedback to your mentor after working together. Write at most 3 sentences in first person. Use the provided aggregate teaching signals to identify one thing that helps you and one helpful adjustment. Never mention scores, verdicts, evaluation, AI, hidden states, or any internal process. Return only the feedback.`,
